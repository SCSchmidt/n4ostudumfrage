...
arch <-  c("Archaeologie", "Prähistorische und Historische Archäologie")
...
d$fach[d$fach %in% arch ] <- "Archaeologie"
...
# versuche des labelns der balkenteile mit anzahl...
...
geom_bar(na.rm = TRUE, position = position_dodge())+
  ...
  scale_fill_manual(values = n4o_farben[c(1,10)],
                    ...
                    name = "Assessment")+
  #geom_text(aes(x = 1, y = 50, label = paste("n = ", (nrow(kurse_intern)/2 ))))+ fehler drin
  theme_minimal()
...
ggplot()+
  ...
  labs(x = "most often used software",
       y = "Count (more than twice)")+
  coord_flip()
```

Comparable to first survey.

### most often used data formats
...
ggplot(data = d_d2)+
  geom_col(aes(x = reorder(dateiformat, Count), y = Count))+
  labs(x = "most often used dateiformat",
       y = "Count (more than twice)")+
  coord_flip()
...
## data formats that are not known
  
```{r}
d %>%
  pivot_longer(dateiformatunbekannt.A1.:dateiformatunbekannt.A15.,
               names_to = "dateiformatu",
               values_to = "dateiformatu_ja",
               values_drop_na = TRUE) %>%
  filter(dateiformatu_ja == "Ja") %>%
  mutate(dateiformatu = recode(dateiformatu,
                           "dateiformatunbekannt.A1." = "xls/xlsx",
                           "dateiformatunbekannt.A2." = "doc/docx",
                           "dateiformatunbekannt.A3." = "pdf", 
                           "dateiformatunbekannt.A4." = "ppt/pptx",
                           "dateiformatunbekannt.A5." = "ods",
                           "dateiformatunbekannt.A6." = "csv",
                           "dateiformatunbekannt.A7." = "odt",
                           "dateiformatunbekannt.A8."= "png",
                           "dateiformatunbekannt.A9." = "jpg",
                           "dateiformatunbekannt.A10." = "tiff",
                           "dateiformatunbekannt.A11." = "geotiff",
                           "dateiformatunbekannt.A12." = "geojson",
                           "dateiformatunbekannt.A13." =  "shp",
                           "dateiformatunbekannt.A14." = "json",
                           "dateiformatunbekannt.A15." = "xml"))%>%
  group_by(dateiformatu)%>%
  summarise(Count = n())%>%
  filter(Count > 2)%>%
ggplot()+
  geom_col(aes(x = reorder(dateiformatu, Count), y = Count))+
  labs(x = "unknown dataformats",
       y = "Count (more than twice)")+
  coord_flip()

```

Äh. Sagt uns eher nischt oder?

## How do ppl manage their own data?

### Als wie geübt schätzen Sie sich im Umgang mit Ihren Dokumenten und Daten ein? 1: sehr unsicher bis 10: sehr sicher 


```{r}
ggplot()+
  geom_bar(aes(d$umgangdaten.SQ001.))+
  geom_text(aes(x = 1, y = 50, label = paste("N/A =", sum(is.na(d$umgangdaten.SQ001.)) )))
```

### Für wie wichtig halten Sie einen professionellen Umgang mit Dateien? 1: unwichtig bis 10: sehr wichtig 
```{r}
ggplot()+
  geom_bar(aes(d$wichtigdateien.SQ001.))+
  geom_text(aes(x = 1, y = 50, label = paste("N/A =", sum(is.na(d$wichtigdateien.SQ001.)) )))

```

###Wie finden Sie Ihre Dokumente und Dateien auf Ihrem Rechner?
Bitte wählen Sie eine der folgenden Antworten:

    meistens mit Hilfe der Suchfunktion
    meistens über meine eigene Ordnerstruktur

```{r}
ggplot()+
  geom_bar(aes(d$findendateien))+
  geom_text(aes(x = 1, y = 50, label = paste("N/A =", sum(is.na(d$findendateien)) )))+
  coord_flip()

```


    Sonstiges: -- eigentlich nur 4x "beides"



### Haben Sie auf Ihrem Rechner eine dem/n Arbeitsziel(en) angepasste Ordnerstruktur konzipiert und angelegt?

`r sum(d$rechnerfolderstruct == "Nein")` x Nein.

### Legen Sie verschiedene Versionen Ihrer Dokumenten und Dateien an?

`r sum(d$dateiversionen == "nein")` x Nein.

```{r}
ggplot()+
  geom_bar(aes(d$dateiversionen))+
  coord_flip()

```
+ 4x Ordnerstruktur nach Version und Dat
...
d %>%
  ...
  filter(Count > 2)%>%
ggplot()+
  geom_col(aes(x = reorder(backup, Count), y = Count))+
...
```{r eval =FALSE}
d %>%
  filter(backupstrategie2.other. !="NA")%>%
  ggplot()+
  geom_bar(aes(x = backupstrategie2.other.))+
  coord_flip()
  
```

A number of students use E-mails for backup ( `r length(grep("mail", tolower(d$backupstrategie2.other.)))` students), whereas others (( `r length(grep("server", tolower(d$backupstrategie2.other.)))` students) use servers offered by the institutions.
(Rest vernachlässigbar)  

`r sum(d$angstdatenverlust == "ja", na.rm=TRUE) ` are afraid of losing data.

`r sum(d$angstdatenverlust == "nein", na.rm=TRUE)` are not afraid of losing data.

`r  sum(d$angstdatenverlust == "dazu habe ich mir bisher keine Gedanken gemacht", na.rm=TRUE)` haven't thought about the topic yet  (NA = `r sum(is.na(d$angstdatenverlust)) `).
...
These "Richtlinien" are known to students: `r  d$richtliniendateien1 `, most, though (`r sum(d$richtliniendateien == "Nein") `) did not know any guidelines.
...
d %>%
  ggplot()+
  geom_bar(aes(x = itaffinitaet.SQ001.))+
  coord_flip()
```
...
`r sum(d$fortbildung == "Nein", na.rm=TRUE)` did not wish for further training. Others, though, `r sum(d$fortbildung == "Ja", na.rm=TRUE)` respondents to be exact, did.
...
d_fb %>%
  group_by(fortbildung_them)%>%
  summarise(Count = n())%>%
  ggplot()+
  geom_col(aes(x = reorder(fortbildung_them, Count), y = Count))+
  coord_flip()
```
